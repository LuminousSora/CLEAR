...
class Trainer:
    ...
    def train(self, dataset, encoder, scl, scl2):
        ...
        with torch.no_grad():
            ...
            for data in train_loader:
                ...
                if n_batches == 0:
                    ...
                    encoded_data_label=labels
                else:
                    encoded_data = torch.cat((encoded_data, encoded), axis=0)
                    encoded_data_label = torch.cat((encoded_data_label, labels), axis=0)
                
                n_batches += 1
        
        return encoded_data.cpu().numpy(), encoded_data_label.cpu().numpy()
